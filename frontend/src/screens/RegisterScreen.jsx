...
export default function RegisterScreen({ onRegisterSuccess, onGoToLogin }) {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(data.detail || 'Registeration Failed.');
            }
            ...
            <main className='flex-1 flex items-center justify-center p-6'>
                <div className='w-full max-w-md'>
                    ...
                    <div className='mb-10 text-center'>
                        <h1 className='font-mono text-3xl font-bold tracking-light mb-2'>Create Account</h1>
                        <p className='text-text-secondary text-sm'>Start tailoring resumes with AI-powered precision.</p>
                    </div>
                    ...
                    <form onSubmit={handleRegister} className='flex flex-col gap-5'>
                        ...
                        <div className='flex flex-col gap-2'>
                            <label className='font-mono text-xs text-text-secondary tracking-widest'>FULL NAME</label>
                            <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder='Jane Doe' required className='w-full bg-surface border border-border rounded-std px-4 py-3 font-mono text-sm text-text-primary placeholder-text-secondary focus:outline-none focus:border-primary transition-colors duration-300' />
                        </div>
                        ...
                        <div className='flex flex-col gap-2'>
                            ...
                            <div className='relative'>
                                <input type={showPassword ? 'text' : 'password'} minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder='Minimum 8 characters' required className='w-full bg-surface border border-border rounded-std px-4 py-3 font-mono text-sm text-text-primary placeholder-text-secondary focus:outline-none focus:border-primary transition-colors duration-300' />
                    ...
                    <div>
                        ...
                        <p className='text-center text-sm text-text-secondary'>
                            ...
                            <button onClick={onGoToLogin} className='text-primary hover:underline font-semibold transition-colors'>
                                Sign In
                            </button>
                        </p>
                    </div>
