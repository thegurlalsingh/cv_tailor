...
graph TD
    ...
    Writer[ Writer Agent<br/>Drafts New Resume] --> Auditor
    Auditor[ Auditor Agent<br/>Fact-checks Draft] --> ATSEval
    ATSEval[ ATS Evaluator Agent<br/>Scores Draft] --> Planner
    Planner[ Revision Planner Agent<br/>Decides Next Steps]
...
2.   **The Writer:** Rewrites the resume JSON according to the strategy, optimizing action verbs and keywords.
3.   **The Auditor (Fact-Checker):** Strictly compares the new draft against the original resume. If the writer hallucinated or fabricated any skills, the Auditor flags it and forces a rewrite.
4.   **The ATS Evaluator:** Simulates an enterprise Applicant Tracking System. Scores the draft out of 100%.
5.   **The Revision Planner:** Analyzes the ATS score and Auditor feedback. If the score is low or facts were hallucinated, it plans the next loop. Otherwise, it approves the final draft.
